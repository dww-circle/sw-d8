...
class SWSubscriber implements EventSubscriberInterface {
  ...
  /**
   * Handles all of SW's custom redirect logic.
   *
  ...
  public function onRequestEarlyRedirect(GetResponseEvent $event) {
    ...
    // Redirect respond/blog/* to the /contact/readers_views form.
    ...
    // Redirect */feed to /recent/rss.xml.
    elseif (preg_match('#^.*/feed$#', $request_uri, $matches)) {
      $response = new RedirectResponse($base_path . 'recent/rss.xml', 301);
    }
    if (isset($response)) {
